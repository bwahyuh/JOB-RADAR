...
def extract_jobs():
    print(f"🚀 STARTING MISSION V2: DEEP DIVE SCRAPING")
    ...
    print(f"📦 Found {len(articles)} job candidates. Starting detail extraction...\n")
    ...
    for i, job in enumerate(articles):
        try:
            ...
            print(f"[{i+1}/{len(articles)}] Processing: {title} ({company})")
    ...
    if job_list:
        ...
        print(df[['title', 'work_type']].tail())
    else:
        print("⚠️ No data found. Check selectors.")
